...
then
    ...
    git push --delete -q https://$GITHUB_OAUTH_TOKEN@github.com/laxmicoinofficial/go snapshots || true
    git tag snapshots -a -m "Generated snapshots from TravisCI for build $TRAVIS_BUILD_NUMBER on $(date --utc +'%F-%T')"
    git push -q https://$GITHUB_OAUTH_TOKEN@github.com/laxmicoinofficial/go --tags
fi
